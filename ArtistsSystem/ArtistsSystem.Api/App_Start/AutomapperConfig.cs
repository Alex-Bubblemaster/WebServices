    ...
    public class AutomapperConfig
    {
        public static void Initialize()
        {
            ...
            Mapper.CreateMap<Album, AlbumRequestModel>()
                .ForMember(al => al.Artists, opt => opt.MapFrom(al => (al.Artists.FirstOrDefault()!= null ? string.Join(", ", al.Artists.Select(x=>x.Name)): "n/a")))
                .ForMember(al => al.Songs, opt => opt.MapFrom(al => (al.Songs.FirstOrDefault() != null ? string.Join(", ", al.Songs.Select(x => x.Title)) : "n/a")));
        }
    }
}